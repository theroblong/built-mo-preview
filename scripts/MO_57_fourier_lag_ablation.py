"""MO_57 — Fourier + Lag2/3 + Price Bin Ablation.

PURPOSE
-------
Four candidates, individually tested at the 0.03pp global-wMAPE promotion threshold.
Champion: MO_53 28-feature set (same baseline as MO_56).

CANDIDATES
----------
  A. Fourier: replace week_of_year (integer 1–52) with week_sin + week_cos.
     Integer encoding implies week 52 is far from week 1 — they are adjacent.
     Fourier encoding (sin(2π·week/52.178), cos(…)) represents cyclical boundary
     correctly and should help Dec→Jan seasonal transitions.
     Feature count: 29 (28 − week_of_year + week_sin + week_cos).

  B. +lag2 (base_units_lag2): fills the 1-week→4-week gap in existing lag set.
     Some SKUs have 2-week inventory/promo cycles not captured by lag1→lag4.
     Feature count: 29.

  C. +lag3 (base_units_lag3): same motivation; some cycles are 3-week.
     Tested SEPARATELY from lag2 (MO_52 showed group testing masks individual winners).
     Feature count: 29.

  D. +price_change_bin (4-level categorical):
     Regime semantics that continuous price features cannot express.
     bins: -1=price increase, 0=stable, 1=small discount, 2=significant discount.
     Encoded as LightGBM categorical_feature.
     Low confidence — all explicit price signals rejected in MO_52/53/54/56.
     Conditional evaluation (event vs stable) per Rule 8.
     Feature count: 29.

  E. +lag2+lag3 combined: only tested if BOTH B and C individually promote.
     Feature count: 30.

CONDITIONAL EVALUATION
----------------------
Same event/stable split as MO_56:
  EVENT rows: weeks_since_launch ≤ 26 OR |arp_pct_change| ≥ 0.05
Promotion criteria (consistent with MO_53/56):
  - Global ≥ 0.03pp improvement, OR
  - Event ≥ 0.05pp improvement AND stable doesn't worsen > 0.05pp

CHAMPION BASELINE
-----------------
MO_53 28-feature set, avg 3-cutpoint CV wMAPE 6.413%.

OUTPUTS
-------
  outputs/mo57_individual_ablation.csv   — per-candidate global + conditional wMAPE
  outputs/mo57_champion_cv.csv           — champion 3-cutpoint CV
  outputs/mo57_rolling_cv_results.csv    — best promoted set 3-cutpoint CV
  outputs/mo57_champion_conditional.csv  — champion event/stable baseline (for cache-skip)
  outputs/v2_mo57_results.png            — global wMAPE bar chart
  outputs/v2_mo57_conditional.png        — event vs stable grouped bar
  outputs/v2_mo57_rolling_cv.png         — rolling CV (if any promoted)
  outputs/v2_mo57_shap.png               — SHAP for best promoted set (if promoted)
  HTML Section 25 patched into outputs/built_demand_intelligence_report.html
"""
...
# Features that should be passed as LightGBM categorical_feature even though they
# are already encoded as integers (not object dtype).
EXTRA_CATEGORICAL = ["price_change_bin"]

GROUP_COLS            = ["upc", "channel_outlet", "retail_account", "geography_raw"]
DEC2025_CUTOFF        = pd.Timestamp("2026-01-01", tz="UTC")
PROMOTE_THRESHOLD     = 0.03   # pp global improvement
...
def _build_candidates():
    fourier_feats = [f for f in CHAMPION_FEATS if f != "week_of_year"] + ["week_sin", "week_cos"]
    return [
        {
            "label": "Fourier(week_sin+week_cos)",
            "feats": fourier_feats,
            "note": (
                "Replace week_of_year integer (1–52) with sin/cos Fourier encoding. "
                "week_of_year implies week 52 is 51 steps from week 1 — they are adjacent. "
                "Fourier terms represent the cyclical boundary correctly. "
                "Requires week_sin + week_cos columns (MO_25 v8)."
            ),
            "is_replacement": True,
        },
        {
            "label": "+base_units_lag2",
            "feats": CHAMPION_FEATS + ["base_units_lag2"],
            "note": (
                "Additive: base_units_lag2 fills the gap between lag1 (adjacent week) "
                "and lag4 (monthly). Some SKUs show 2-week inventory or promotional cycles "
                "that lag1→lag4 brackets too coarsely."
            ),
            "is_replacement": False,
        },
        {
            "label": "+base_units_lag3",
            "feats": CHAMPION_FEATS + ["base_units_lag3"],
            "note": (
                "Additive: base_units_lag3 — same motivation as lag2. "
                "Tested separately (MO_52 showed group testing masks individual winners). "
                "If BOTH lag2 and lag3 promote individually, a combined test is added."
            ),
            "is_replacement": False,
        },
        {
            "label": "+price_change_bin(cat)",
            "feats": CHAMPION_FEATS + ["price_change_bin"],
            "note": (
                "4-level price regime categorical (-1=increase, 0=stable, 1=small_discount, "
                "2=significant_discount). Regime semantics that continuous price features "
                "cannot express. All prior explicit price signals rejected (MO_52/53/54/56). "
                "Low confidence but worth a single test. Evaluated on event-context split."
            ),
            "is_replacement": False,
        },
    ]
...
def event_mask_for(df):
    ...
    price  = (df["arp_pct_change"].abs() >= EVENT_PRICE_THRESHOLD
              if "arp_pct_change" in df.columns
...
def _encode_categoricals(train_df, val_df, test_df, af):
    """Encode object-dtype columns as integers for LightGBM.
    Returns (tr, va, te, cat_feat_list) where cat_feat_list includes
    both object-encoded features AND EXTRA_CATEGORICAL features present in af.
    """
    ...
    # Also declare integer-encoded categorical columns for LightGBM
    extra_cats = [c for c in EXTRA_CATEGORICAL if c in af]
    return tr, va, te, cat_feats + extra_cats
...
def rolling_cv_global(df, feats, label, params=None):
    """3-cutpoint CV (global wMAPE only)."""
    cutpoints = [
...
def chart_global_results(results, champion_wmape, out_path):
    ...
    ax.set_title(f"MO_57: Fourier + Lag2/3 + Price Bin — Global wMAPE\n"
...
def chart_conditional(results, champ_global, champ_event, champ_stable, out_path):
    ...
    ax.set_ylabel("wMAPE (%)")
    ax.set_title(f"MO_57: Conditional Accuracy — Event-Context vs Stable\n"
...
def chart_rolling_cv(cv_champ, cv_prom, label, out_path):
    ...
    ax.set_ylabel("wMAPE (%)")
    ax.set_title("MO_57: Rolling 3-Cutpoint CV — Champion vs Promoted")
    ax.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=130)
    plt.close()
...
def chart_shap(model, test_df, feats, out_path):
    ...
    plt.title("MO_57: SHAP Feature Importance — Promoted Feature Set")
    plt.tight_layout()
...
def build_html_section25(results, chart_paths, cv_champ, cv_prom,
    ...
    rows_html = ""
    for r in results:
        if np.isnan(r["global_wmape"]):
            gd, ed, sd = float("nan"), float("nan"), float("nan")
        else:
            gd = r["global_wmape"] - champ_global
            ed = (r["event_wmape"]  - champ_event)  if not np.isnan(r.get("event_wmape", float("nan")))  else float("nan")
            sd = (r["stable_wmape"] - champ_stable) if not np.isnan(r.get("stable_wmape", float("nan"))) else float("nan")
        gc = "#27ae60" if (not np.isnan(gd) and gd <= -PROMOTE_THRESHOLD) else ("#e74c3c" if (not np.isnan(gd) and gd > 0) else "#888")
        ...
        g_str  = f"{r['global_wmape']:.3f}%" if not np.isnan(r.get("global_wmape", float("nan"))) else "n/a"
        gd_str = f"{gd:+.3f}pp" if not np.isnan(gd) else "n/a"
        ev_str = f"{r['event_wmape']:.3f}%"  if not np.isnan(r.get("event_wmape", float("nan")))  else "n/a"
        ed_str = f"{ed:+.3f}pp" if not np.isnan(ed) else "n/a"
        sv_str = f"{r['stable_wmape']:.3f}%" if not np.isnan(r.get("stable_wmape", float("nan"))) else "n/a"
        sd_str = f"{sd:+.3f}pp" if not np.isnan(sd) else "n/a"
        rows_html += f"""
        <tr>
          <td style='padding:.4rem .7rem'>{r['label']}{prom_mark}</td>
          <td style='padding:.4rem .7rem;text-align:right'>{g_str}</td>
          <td style='padding:.4rem .7rem;text-align:right;color:{gc}'>{gd_str}</td>
          <td style='padding:.4rem .7rem;text-align:right'>{ev_str}</td>
          <td style='padding:.4rem .7rem;text-align:right;color:{ec}'>{ed_str}</td>
          <td style='padding:.4rem .7rem;text-align:right'>{sv_str}</td>
    ...
    champ_tag = "★ New champion" if is_new_champion else "Champion unchanged (MO_53)"
    delta_cv  = avg_cv_champ - avg_cv_prom if avg_cv_prom else float("nan")
    n_feats   = len(promoted_feats)
...
<section style='font-family:sans-serif;max-width:1100px;margin:3rem auto;padding:0 1rem'>
  <h2 style='font-size:1.4rem;border-bottom:2px solid #2c3e50;padding-bottom:.5rem'>
    Section 25 — MO_57: Fourier Week Encoding, Lag2/3, Price Change Bins
  </h2>
  <p style='color:#555;font-size:.9rem'>
    Four feature engineering candidates, each tested individually at the 0.03pp global
    promotion threshold against the MO_53 28-feature champion. Champion baseline:
    Global={champ_global:.3f}%  Event-context={champ_event:.3f}%  Stable={champ_stable:.3f}%.
    <br><strong>Fourier</strong>: replaces integer week_of_year (week 52 ≠ week 1 in integer space)
    with sin/cos encoding that correctly represents the Dec→Jan cyclical boundary.
    <strong>Lag2/3</strong>: fills the 1-week to 4-week gap in existing AR lag set.
    <strong>Price bin</strong>: 4-level regime categorical (-1/0/1/2) — low confidence,
    all prior continuous price signals rejected. {champ_tag}.
  </p>
  ...
  <div style='display:flex;gap:1.5rem;flex-wrap:wrap;margin-top:1rem'>
    ...
    <div style='background:#f0f9f0;border-left:4px solid #27ae60;padding:.7rem 1rem;flex:1;min-width:180px'>
      ...
      <span style='font-size:1.1rem;color:{"#27ae60" if not np.isnan(delta_cv) and delta_cv>0 else "#e74c3c"}'>{avg_cv_prom:.3f}%</span>
      &nbsp;<small>({delta_cv:+.3f}pp avg CV vs champion)</small><br>
      <small>{promoted_label} — {n_feats} features</small>
    </div>
  </div>

  <h3 style='margin-top:1.5rem'>25.1 Individual Candidate Results — Global + Conditional (Dec 2025)</h3>
  <p style='color:#555;font-size:.88rem'>
    Champion baseline: Global={champ_global:.3f}%  Event={champ_event:.3f}%  Stable={champ_stable:.3f}%.
    ✓ = promoted (global ≥{PROMOTE_THRESHOLD}pp OR event ≥{PROMOTE_EVENT_THRESH}pp with
    stable not worsening &gt;{PENALIZE_STABLE_CAP}pp).
    Fourier candidate replaces week_of_year; all others are additive.
  </p>
  ...
  <h3 style='margin-top:1.5rem'>25.2 Conditional Accuracy: Event-Context vs Stable Series</h3>
  <p style='color:#555;font-size:.88rem'>
    Event-context rows: wsl ≤ {EVENT_LAUNCH_WINDOW} weeks (launch window) OR
    |Δprice| ≥ {EVENT_PRICE_THRESHOLD*100:.0f}% (significant price move). Stable rows: all others.
  </p>
  {'<img src="' + imgs["conditional"] + '" style="width:100%;margin-top:.5rem">' if "conditional" in imgs else ""}

  {'<h3 style="margin-top:1.5rem">25.3 Rolling 3-Cutpoint CV — Champion vs Promoted</h3>' +
  ...
  {'<h3 style="margin-top:1.5rem">25.4 SHAP Feature Importance — Promoted Set</h3><img src="' + imgs["shap"] + '" style="width:100%;margin-top:.5rem">' if "shap" in imgs else ""}

  <h3 style='margin-top:1.5rem'>25.5 Feature Notes</h3>
  <ul style='font-size:.9rem;color:#333'>
    <li><strong>Fourier encoding</strong>: week_sin = sin(2π × week_of_year / 52.178),
        week_cos = cos(…). The divisor 52.178 = 365.25/7 (exact tropical year in weeks).
        Cyclical adjacency: week 52 and week 1 map to nearly identical (sin, cos) values.
        Integer week_of_year cannot express this — a tree split on "week &lt; 52" will
        never learn that Dec→Jan is seasonal continuity.</li>
    <li><strong>Lag2/3</strong>: fills the gap between lag1 (t−1) and lag4 (t−4).
        Tested separately per MO_52 lesson: group testing masked individual contribution.
        If both promote, a combined lag2+lag3 test is appended above.</li>
    <li><strong>price_change_bin</strong>: -1=increase (&gt;$0.05), 0=stable (|Δ$|&lt;$0.05 AND |Δ%|&lt;2%),
        1=small discount ($0.05–$0.25 or 2–5%), 2=significant discount (&gt;$0.25 or &gt;5%).
        Passed as LightGBM categorical_feature. ~90–95% of rows are bin=0; distribution
        imbalance is the main risk. All prior continuous price signals rejected — expectation is rejection here too.</li>
  </ul>
</section>"""
    return section
...
if __name__ == "__main__":
    print("=" * 70)
    print("MO_57 — Fourier + Lag2/3 + Price Bin Ablation")
    print("=" * 70)

    # ── Cache-skip ─────────────────────────────────────────────────────────────
    _ABLATION_CSV = Path(OUTPUT_DIR) / "mo57_individual_ablation.csv"
    _CHAMP_CV_CSV = Path(OUTPUT_DIR) / "mo57_champion_cv.csv"
    _PROM_CV_CSV  = Path(OUTPUT_DIR) / "mo57_rolling_cv_results.csv"
    _COND_CSV     = Path(OUTPUT_DIR) / "mo57_champion_conditional.csv"
    _PNG_MAIN     = Path(OUTPUT_DIR) / "v2_mo57_results.png"
    ...
    if all(p.exists() for p in [_ABLATION_CSV, _CHAMP_CV_CSV, _PROM_CV_CSV, _COND_CSV, _PNG_MAIN]):
        ...
        _promoted_rows = results_df[results_df.get("promoted", pd.Series(False)) == True]
        if len(_promoted_rows) > 0:
            _best_label  = _promoted_rows.iloc[0]["label"]
            _best_feats  = CHAMPION_FEATS  # loaded from CSV; feats column may not be present
        else:
            _best_label  = "Champion (no promotion)"
            _best_feats  = CHAMPION_FEATS
        _chart_paths = {k: str(Path(OUTPUT_DIR) / v) for k, v in {
            "global_results": "v2_mo57_results.png",
            "conditional":    "v2_mo57_conditional.png",
            "rolling_cv":     "v2_mo57_rolling_cv.png",
            "shap":           "v2_mo57_shap.png",
        }.items() if (Path(OUTPUT_DIR) / v).exists()}
        _valid_results = results_df.to_dict("records")
        for r in _valid_results:
            for k in ("global_wmape", "event_wmape", "stable_wmape"):
                if k in r:
                    r[k] = float(r[k]) if not pd.isna(r[k]) else float("nan")
        _sec = build_html_section25(
            _valid_results, _chart_paths, cv_champ, cv_prom,
            _best_label, _champ_global, _champ_event, _champ_stable,
            _best_feats, _avg_cv_champ, _avg_cv_prom,
            bool(_avg_cv_prom < _avg_cv_champ - PROMOTE_THRESHOLD),
        )
        patch_html(_sec)
        print("MO_57 COMPLETE (cached)")
        sys.exit(0)
    ...
    v8_cols = ["week_sin", "week_cos", "base_units_lag2", "base_units_lag3", "price_change_bin"]
    for col in v8_cols:
        if col in df.columns:
            if col == "price_change_bin":
                dist = df[col].value_counts().to_dict()
                print(f"  {col}: present  distribution={dist}")
            else:
                nz = (df[col].abs() > 0.001).sum()
                print(f"  {col}: present  {nz:,} nonzero ({nz/len(df)*100:.1f}%)")
        else:
            print(f"  {col}: MISSING — run MO_25 v8 first")
    ...
    # Save champion conditional for cache-skip
    pd.DataFrame([
        {"subset": "global",  "wmape": champ_global},
        {"subset": "event",   "wmape": champ_event},
        {"subset": "stable",  "wmape": champ_stable},
    ]).to_csv(_COND_CSV, index=False)
    ...
    for cand in CANDIDATES:
        ...
        if missing:
            ...
            results.append({
                "label": label, "note": cand["note"],
                "global_wmape": np.nan, "event_wmape": np.nan,
                "stable_wmape": np.nan, "promoted": False,
            })
            continue

        g_wm, e_wm, s_wm, _ = train_eval_conditional(tr_dec, va_dec, te_dec, feats)
    ...
    # ── 3b. Conditional combined test for lag2+lag3 ───────────────────────
    lag2_result = next((r for r in results if r["label"] == "+base_units_lag2"), None)
    lag3_result = next((r for r in results if r["label"] == "+base_units_lag3"), None)
    if (lag2_result and lag3_result and
            lag2_result.get("promoted") and lag3_result.get("promoted")):
        print("\n  Both lag2 AND lag3 promoted — testing combined set …")
        combined_feats = CHAMPION_FEATS + ["base_units_lag2", "base_units_lag3"]
        missing_comb   = [f for f in combined_feats if f not in df.columns]
        if not missing_comb:
            g_wm, e_wm, s_wm, _ = train_eval_conditional(
                tr_dec, va_dec, te_dec, combined_feats
            )
            g_delta = g_wm - champ_global
            e_delta = (e_wm - champ_event)  if not np.isnan(e_wm)  else float("nan")
            s_delta = (s_wm - champ_stable) if not np.isnan(s_wm)  else float("nan")
            promoted, reason = should_promote(g_delta, e_delta, s_delta)
            marker = f" ✓ PROMOTED ({reason})" if promoted else f" (Δglobal={g_delta:+.3f}pp)"
            print(f"  +lag2+lag3 combined: Global={g_wm:.3f}% ({g_delta:+.3f}pp) | "
                  f"Event={e_wm:.3f}% ({e_delta:+.3f}pp) | "
                  f"Stable={s_wm:.3f}% ({s_delta:+.3f}pp){marker}")
            results.append({
                "label": "+lag2+lag3(combined)",
                "note":  "Combined test: only run because both lag2 and lag3 individually promoted.",
                "global_wmape": float(g_wm), "event_wmape": float(e_wm),
                "stable_wmape": float(s_wm), "promoted": promoted,
                "promote_reason": reason,
            })

    results_df = pd.DataFrame(results)
    results_df.to_csv(_ABLATION_CSV, index=False)
    ...
    if promoted_results:
        ...
        # Look up full feats list from CANDIDATES (or use combined)
        best_cand = next((c for c in CANDIDATES if c["label"] == best["label"]), None)
        if best_cand:
            best_feats = best_cand["feats"]
        elif best["label"] == "+lag2+lag3(combined)":
            best_feats = CHAMPION_FEATS + ["base_units_lag2", "base_units_lag3"]
        else:
    ...
    cv_champ.to_csv(_CHAMP_CV_CSV, index=False)
    cv_prom.to_csv(_PROM_CV_CSV,  index=False)
    ...
    shap_path = os.path.join(OUTPUT_DIR, "v2_mo57_shap.png")
    if best_model:
        try:
    ...
    is_new_champion = update_model_history(
        f"MO_57 promoted ({best_label})",
        best_global, avg_cv_prom, n_dec, CHAMPION_PARAMS, best_feats,
    )

    # ── 7. Charts ─────────────────────────────────────────────────────────────
    print("\n[7] Generating charts …")
    valid_results = [r for r in results if not np.isnan(r.get("global_wmape", float("nan")))]
    path_global = os.path.join(OUTPUT_DIR, "v2_mo57_results.png")
    path_cond   = os.path.join(OUTPUT_DIR, "v2_mo57_conditional.png")
    path_cv     = os.path.join(OUTPUT_DIR, "v2_mo57_rolling_cv.png")
    ...
    section_html = build_html_section25(
    ...
    if not promoted_results:
        print("    None — champion unchanged (MO_53)")
    print(f"\nNext: if Fourier promoted → update MO_26 FEATURE_COLS (drop week_of_year, add week_sin/cos).")
    print(f"      if lag2/3 promoted → add to FEATURE_COLS.")
    print(f"      Then re-run MO_26 → MO_27 → Druid ingest.")
